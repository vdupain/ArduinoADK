...
public class RemoteControlServer implements Runnable {
	...
	public RemoteControlServer(UsbAccessoryManager usbAccessoryManager, Handler handler, int port) {
	...
	public void service(int port) {
		...
		try {
			...
			ArduinoManager arduinoManager = new ArduinoManager(usbAccessoryManager, handler);
			log("Accept client connection...");
			while (!stopServer.get()) {
				try {
					Log.d(TAG, "Waits for " + serverSocket.getSoTimeout() + "ms an incoming request...");
					pool.execute(new RemoteControlClientHandler(serverSocket.accept(), this.handler, arduinoManager));
